...
const sourceDir = '/Users/sawkhaing/Documents/codebase/telegram_media_downloader/myanmarbooksforkindle/document/';
...
Object.keys(bookGroups).forEach(baseName => {
  ...
  if (baseName.includes('_')) {
    ...
    if (parts.length >= 2) {
      author = parts.slice(-2).join(' ').replace(/_/, ' ');
  ...
  // Copy all format files and generate format array
  const bookFormats = formats.map(f => {
    const destPath = path.join(publicBooksDir, f.file);
    fs.copyFileSync(path.join(sourceDir, f.file), destPath);
  ...
  newBooks.push({
    ...
    popular: isPopular,
    formats: bookFormats // The new array of formats!
  });
});

console.log(`Processed ${newBooks.length} grouped books.`);
...
console.log('books-data.js updated successfully!');
